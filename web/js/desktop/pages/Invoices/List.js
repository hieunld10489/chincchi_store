...
//                Ext.getCmp('invoice_number').setValue('');
            ...
            handler: function () {
                ...
                MyUx.grid.Printer.printAutomatically = false;
                var arrSelectedRecord   = [],
                    selectedRecordValue = Ext.getCmp('invoiceId').getValue();
                Ext.each(storeListOutput.data.items, function (record) {
                    if(record.data.id == selectedRecordValue) {
                        arrSelectedRecord.push(record);
                        return;
                    }
                });

                MyUx.grid.Printer.printExtList(arrSelectedRecord, selectedRecordValue);
            }
        }, {
            xtype: 'button',
